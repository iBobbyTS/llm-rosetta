...
7. 对所有“可能没有变化”和“有变化”的兼容点调用真实 API 测试：观测 Codex/GPT
   原生请求形状时使用 `gpt-5.6-terra`，调试第三方模型转换时默认使用低成本的
...
- Responses Lite `additional_tools`、developer instructions、`reasoning.context=all_turns`
  与嵌入式工具过滤/去重；
...
- custom/freeform `apply_patch` 与 code-mode `exec` 定义、grammar、delta 拼接、call/output
  和 fallback command；其中 `exec` 的 Chat 降级回程必须恢复为 `custom_tool_call`，不得
  把不合规 JSON function 参数猜测改写为 JavaScript；
...
按调试目标选择模型，不能只看 Codex-facing alias：

- 需要观测 Codex/GPT 原生 request、tool、reasoning、transport 或 stream shape 时，
  使用 `gpt-5.6-terra`，并由 Rosetta trace 确认上游确实是 GPT route；转发到第三方
  provider 的同名 alias 不能作为 GPT 原请求证据；
- 调试第三方模型的 Responses→Chat bridge、reasoning 和多轮工具调用时，默认使用
  `deepseek-v4-flash`，因为成本较低；
- 版本升级实际测试应至少包含上述两类中与本次变化相关的场景；
...
- model catalog 启用 code mode 时，实际验证 `exec/wait` 和 nested tool continuation；
  特别检查 `exec` 收到的 payload 是 custom/raw-source，而不是 function/JSON payload，
  并确认第三方模型误用 freeform 工具时以可见工具错误恢复、不会 fatal；
