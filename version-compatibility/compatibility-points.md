...
| Responses Lite / `additional_tools` | Responses→Responses 可原样透传；Responses→Chat 合并顶层与 `input[].type=additional_tools` 的工具，保留 developer instructions、按最终 Chat 名称去重 | `converters/openai_responses/message_ops.py`, `converter.py`, `gateway/proxy.py` | 高；0.144.0 model catalog 已为部分模型启用 Responses Lite，工具和 developer instructions 的位置会改变 |
| custom/freeform 工具 | 识别 Responses `type: custom` 的 `apply_patch` 和 Code Mode `exec`，转换成 Chat 可调用形式后，在响应回程恢复 Codex-native tool type、call/output | `openai_responses/converter.py`, `openai_responses/tool_ops.py`, `gateway/tool_adaptation.py` | Codex 改 custom grammar、call/output/delta event，或第三方模型把 freeform `exec` 误解成 JSON shell function |
...
| Responses Lite / `additional_tools` | 精确回放 Lite request；提取嵌入工具与 developer instructions；覆盖顶层/嵌入工具混用、去重、`reasoning.context=all_turns`、`parallel_tool_calls=false` 和嵌入式 image-generation 移除 | 为 `deepseek-v4-flash` 使用受控 catalog override 启用 Lite，完成真实多轮工具调用并确认第二轮能消费结果 |
| custom/freeform 工具 | `apply_patch` schema/grammar/delta/call-output round-trip；Code Mode `exec` 在 Responses→Chat→Responses 的非流式、added/delta/done/completed 回程均恢复为 `custom_tool_call`；保留不合规第三方参数，不猜测改写为 JavaScript | 真实 Codex 执行成功 patch、失败 patch 后修正续轮；为启用 code mode 的 catalog 执行 `exec/wait` 和 nested tool call，确认工具失败可见且不会出现 fatal incompatible-payload |
...
### Responses Lite 与 `additional_tools`

Codex 0.144.0 的 bundled model catalog 已为部分模型启用 `use_responses_lite=true`。
在该模式下，Codex 不再把工具放在顶层 `tools`：它会在 `input` 开头插入
`type: "additional_tools"` item，并用 developer message 携带原本的 instructions；
reasoning 同时可能使用 `context: "all_turns"`。

Responses→Responses direct path 能自然保留这种 body。Responses→Chat 当前会忽略
未知且不带扩展 namespace 的 `additional_tools` item，而顶层 `tools` 又为空，结果是
Chat upstream 收不到任何工具定义。`remove_image_generation` 目前也只检查顶层工具，
无法处理 Lite item 中的 `image_gen` namespace/function。这是 0.144.0 升级的确定兼容
缺口，修复和真实多轮工具测试完成前不能声明支持启用 Responses Lite 的 model catalog。
...
`custom_tool_call_output`。启用 code mode 的 catalog 还会以同一 wire 类型暴露 `exec`，
其 `input` 必须是 raw JavaScript source，而不是 shell 参数对象。Rosetta 同时维护两层
兼容：
...
当 Chat upstream 把 custom/freeform tool 降级为普通 function call 后，Responses 回程
必须按请求期记录的 `metadata.provider_type="custom"` 恢复 `custom_tool_call`；这同时
适用于非流式 response 和 streaming 的 added/delta/done/completed。不能把第三方模型
返回的 `{"cmd": "..."}` 擅自合成为 JavaScript：它是模型未遵守 freeform 语义的证据，
应由 Codex 作为可见工具错误处理并让模型重试，而不是让 Rosetta 猜测执行意图。
...
### 尚未实现 Responses WebSocket 和 `/responses/compact`

当前 gateway 的 Codex surface 是 HTTP `/v1/responses` + SSE。源码中的
Responses WebSocket `response.create`、增量 `previous_response_id` 和远程
`/responses/compact` 都不是已验证能力。Responses Lite 的当前状态单独记录在上文：
direct path 可自然保留，但 bridge 已确认存在兼容缺口。

Codex model/provider 配置不得在没有测试的情况下声明这些能力；每次升级必须确认
Codex 对自定义 provider 仍会使用 HTTP/SSE，或能可靠 fallback。

当前 Responses→Chat 还依赖 Codex 重发完整 input/history 的行为。即使补上
`additional_tools`，若 Codex 开始默认使用 WebSocket/HTTP 增量请求和
`previous_response_id`，Rosetta 仍没有对应的服务端 Responses 会话存储，bridge 会
缺少历史。这一项必须通过真实请求捕获判断，不能仅从 request type 中存在
`previous_response_id` 推断已经启用。
