...
- `Remove image_generation`（移除 image_generation）：从请求中移除 `image_generation` 工具，适用于不应看到该工具的模型。
...
- `Write` 变为一个通过 base64 安全 Python 辅助函数写入 UTF-8 内容的 `exec_command`。
- `Edit` 通常变为自定义的 `apply_patch` 调用。
- `Edit(replace_all=true)` 变为一个执行受控全局替换操作的 `exec_command`。

如果原始请求没有暴露自定义的 `apply_patch`，Rosetta 会回退到 `exec_command` 或 `shell_command`，在可用时通过 heredoc 调用 `apply_patch`。
