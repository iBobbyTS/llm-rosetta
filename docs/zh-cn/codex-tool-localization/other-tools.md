...
只要至少成功投影出一个模型可见的嵌套 Function，Rosetta 就会从最终发给 Chat 模型的工具列表中移除父级 `exec`。父级声明仍在内部用于投影和反向翻译。如果没有任何模型可见声明能够成功解析，Rosetta 会保留原始 `exec`，避免静默删除全部命令能力。
...
Chat Default 会保持 `image_gen__imagegen` 为 Disabled；需要先复制 Profile、把该 Function 设为 Modified 并填写图片端点凭据，才会向上游模型投影。
