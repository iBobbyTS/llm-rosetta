...
| `supports_search_tool` | 布尔值，`false` | 控制 `tool_search` 等 Codex namespace/tool discovery；它与 hosted `web_search`、独立 `web.run` 不同。 | 只有客户端可见的发现流程和 Rosetta namespace 恢复都经过测试时才启用。hosted search 在 Tool Profile 中配置，Modified `web.run` 使用全局联网搜索设置。 |
