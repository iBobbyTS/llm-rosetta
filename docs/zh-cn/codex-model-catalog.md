...
preset 为 DeepSeek V4、GLM 5.2、各 Qwen 3.7 变体、MiMo V2.5、MiniMax M3、Kimi
K2.7 Code 和 Kimi K3 声明已审查值；Rosetta 为未声明此字段的 preset 以及 `gpt-5.2`、
...
`mimo-v2.5`、`mimo-v2.5-pro`、`minimax-m3`、`kimi-k2.7-code` 和 `kimi-k3` 预设。只有 LLM 模型组中
...
| `apply_patch_tool_type` | `"freeform"` 或 null | 非 null 时注册 custom/freeform `apply_patch`。 | 面向最新模型的 Terra 兼容配置固定使用 `freeform`；Rosetta 会为 Chat 上游本地化 custom/freeform 调用。 |
| `tool_mode` | `"direct"`、`"code_mode"`、`"code_mode_only"` 或 null | 选择直接原生工具、同时提供 Code Mode，或只提供 Code Mode。非法 selector 字符串会反序列化为 null。 | 支持范围固定使用 `code_mode_only`；Responses Lite/custom `exec` 到 Chat 的投影由 Rosetta 负责。 |
...
| `supports_search_tool` | 布尔值，`false` | 控制原生 `tool_search` 等 Codex namespace/tool discovery；它与 hosted `web_search`、独立 `web.run` 不同。 | 面向最新模型的 Terra 兼容配置固定为 true。Responses→Chat Code Mode 路径会在实时 `exec` 描述声明 deferred tools 时，投影基于当前请求 `ALL_TOOLS` 的搜索与精确读取。成对的 Node REPL 精确读取会授权结构化 dispatcher 调用，再转回 `exec`；Rosetta 不保留 Gateway namespace/discovery cache。Modified `web.run` 使用全局联网搜索设置。 |
| `web_search_tool_type` | `"text"` 或 `"text_and_image"` | 对 hosted `web_search`，`text` 不发送 `search_content_types`；`text_and_image` 发送 `["text", "image"]`。Responses Lite 会在读取该字段前屏蔽 hosted tools。 | 保持 Terra 的 `text_and_image`。在 Rosetta 强制使用的 Responses Lite 路径上它处于休眠状态：搜索走独立 namespace `web.run`，该字段不配置其 provider 或输出模态。 |
...
本 catalog 只面向当前最新的第三方模型，不提供弱模型 fallback。Rosetta 暴露统一的
Terra 兼容客户端表面，并把固定的 Responses Lite 请求形态转换到模型组配置的 Chat 协议。

| 能力 | 固定基线 | 含义 |
| --- | --- | --- |
| 工具表面 | `tool_mode: "code_mode_only"`、`apply_patch_tool_type: "freeform"` | Codex 暴露 Terra Code Mode 表面，Rosetta 把 custom tools 转换到 Chat。 |
| Responses 方言 | `use_responses_lite: true` | additional tools、developer instructions、header、custom `exec`、Search、compact 和 stream continuation 都由 Rosetta 负责兼容。 |
...
| Compact 兼容 | 每模型 `comp_hash` | 根据上游模型名选择已审查的 compact 兼容组。 |

为新第三方模型做适配时，应保留这套表面，只增加模型自己的身份、限制、模态、
推理档位和 `comp_hash`。不要增加 `direct`、非 Lite、禁用 collaboration 或 hosted-search
fallback 预设；不能使用这套表面的模型不属于本 catalog 的支持范围。

### 当前第三方条目示例

下面的共享值展示固定的 Terra 兼容客户端表面；身份、限制、模态、推理档位和 hash
仍由每个模型声明：
...
版本相关的 schema/打包值差异、consumer 追踪、第三方默认值审查和真实客户端
门禁，统一维护在权威的
[`Codex 版本兼容性 checklist`](../dev/version-compatibility/upgrade-checklist.md)。
本页只保留面向用户的字段参考；当 checklist 发现模型目录契约变化时，应同步更新本页。
