...
| Codex model catalog | Treat the bundled catalog and local catalog overrides as Codex client capability declarations, while Rosetta model groups remain the routing source of truth for alias, upstream model, protocol, and Tool Profile. Default-enabled local mode packages the eight Codex 0.144.1 presets, preserves them in source order, adds sorted non-built-in LLM aliases by deep-copying `gpt-5.6-terra`, excludes Embedding aliases, and transactionally maintains the selected Codex Home's `model_catalog.json` plus root `model_catalog_json`. CLI and WebUI first-use confirmation, disable/clear, startup rebuild, and model-group mutations share this file owner. The field reference covers every current catalog input, including the defaulted `effective_context_window_percent`, four bundled JSON keys ignored by Codex 0.144.1, prompt/template precedence, and safe third-party defaults | `gateway/codex_models_0_144_1.json`, `gateway/local_mode.py`, `gateway/cli.py`, `gateway/config.py`, Admin config/UI, `test_local_mode.py`, `test_cli_local_mode.py`, `test_admin_config_routes.py`, `../openai-codex-src/codex-rs/models-manager/models.json`, `../openai-codex-src/codex-rs/protocol/src/openai_models.rs`, `docs/en/codex-model-catalog.md`, `docs/zh-cn/codex-model-catalog.md` | A Codex upgrade changes fields, nested types, enum values, serde defaults/skip behavior, fallback metadata, bundled model count/order/values, prompt precedence, catalog file loading, `model_catalog_json` semantics, or the requests/tools selected by those values; aliases copied from `gpt-5.6-terra` may then advertise unsupported third-party behavior |
...
| Code tool localization | Inject selected `Read`/`Edit`/`Write`/`Glob`/`Grep` definitions and translate responses back to Codex-native calls. The bundled Chat Default Profile passes through `exec_command` and `write_stdin`, disables `shell_command`, and does not define or inject `Bash`; persisted historical Bash mappings remain readable | `gateway/tool_adaptation.py`, `gateway/admin/tool_catalog.json`, `test_tool_adaptation.py`, `test_admin_tools_catalog.py` | Tool name, parameter schema, call id, execution result format, or Chat Default Profile state changes |
...
| GPT relay provider identity | Codex gates sequential-cutoff reasoning delivery and internal ChatGPT item metadata on provider display name `OpenAI`; request Zstd and current-model compact fallback additionally require Codex-backend auth. The provider-neutral real-service A/B suite sends both identities through the same selected relay and labels synthetic backend-auth cells | `tests/integration/gpt_relay/`, `../openai-codex-src/codex-rs/core/src/session/turn.rs`, Codex request client and history normalization | Codex changes `is_openai`, auth classification, request compression, metadata clearing, reasoning-summary delivery, remote compact error classification, or fallback order; a relay accepts ordinary API-key requests but rejects an OpenAI-identity wire variant |
| Model-group tool profiles | Select bundled `builtin`, `responses_pass_through`, or `responses_web_run_mapping` Profiles, or a complete user Profile, for every LLM model group. Bundled tool delivery states are immutable, while their Function/Hosted card fields can be explicitly saved as input-only `tool_profile_input_overrides` without modifying the packaged JSON. Chat Default disables legacy `multi_agent_v1`; any Disabled Namespace forces all child Function states to Disabled and locks their selectors. Tool Mapping only defaults to Responses pass through; the web.run preset changes only `namespace.web.run` and selects the local `/alpha/search` mapping. Supported paths apply per-tool disabled/passthrough/modified, namespace disabled/passthrough/expanded, and injection disabled/injected states. Function and Hosted items may define multiple localized text/password inputs or select inputs with distinct labels and persisted values. Input `visible_when` and item `description_visible_when` control state-based display without deleting values; descriptions default to every supported state. Values remain inert unless a runtime feature explicitly consumes them. Modified `image_gen.imagegen` consumes its OpenAI Images Base URL/Token, while Modified `web_search` and `web.run` consume their independent search Provider/Token. Bundled Profiles omit hosted `image_generation` | `gateway/tool_profiles.py`, `gateway/config.py`, `gateway/codex_auxiliary.py`, Admin tools/model-group UI, `gateway/proxy.py` | Codex tool IDs, input-definition IDs/types/defaults/options/visibility, supported transformations, namespace expansion, injected localization dependencies, or Responses processing-mode semantics change |
| Static tool catalog | Package the fixed tool inventory, policy capabilities, optional Profile inputs/mutations, and immutable Chat Default Profile source, bound to Codex CLI `0.144.0` and source commit `2e8c3756f95789c215d9ea9a5ade6ec377934b3f`; exclude `tool_search`, obsolete hosted `image_generation`, and runtime-dynamic MCP/plugin/app/connector tools except the GitHub Namespace Profile rule, which applies only when that namespace is present | `src/codex_rosetta/gateway/admin/tool_catalog.json` | Fixed tool sets, namespace members, profile input definitions, wire types, aliases, or model-controlled availability change |
...
| Codex model catalog | Diff the complete bundled JSON key set and per-model values; verify the packaged eight-preset asset byte-for-byte against its bound source; extract `ModelInfo`, nested structs, enum wire values, serde rename/default/skip behavior, unknown-model fallback, instruction-template precedence, and each field's runtime consumers. Test local-mode defaults and confirmation, custom Codex Home, CLI/WebUI enable/disable, stable alias generation, Embedding exclusion, TOML preservation, managed-only cleanup, model mutation synchronization, three-file rollback, remote-host warning, and wheel/sdist resource inclusion; verify the English/Chinese field inventories remain complete and synchronized | Start Codex against a generated catalog, restart it after a WebUI model mutation, and confirm the eight presets plus expected aliases are loaded. For every changed capability selector, run the real client against the actual upstream behind the Rosetta alias and inspect Gateway Logs. Verify the declared command/tool mode, Responses Lite shape, reasoning, context/compact behavior, search, and multi-agent lifecycle. Remote clients must be configured separately. For a new integration, test `web.run` and collaboration v2 first; retain `web_search` or `multi_agent_v1` only when the newer surface is demonstrably unreliable |
...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, bundled input-only override validation, model-group resolution for every protocol, per-tool filtering, multi-value Function/Hosted text/password/select input persistence, select-option validation, input/description state visibility, namespace pass-through/expansion, Disabled Namespace child-state coercion and selector locking, injected-tool selection, absence of hosted `image_generation`, and `web.run`/`image_gen.imagegen` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Chat Default, and a restrictive user Profile; verify bundled card fields survive explicit save/reset while their delivery states reject edits, Chat Default disables `multi_agent_v1` and every child, direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, card inputs survive copy/save/reset and retain values while hidden, descriptions default to all states and honor overrides, search cards use their own Tavily Tokens, select labels display while values persist, disabling a Namespace disables and locks all child Functions, disabled tools disappear, Modified `image_gen.imagegen` selects its OpenAI Images endpoint without leaking its token, and supported adaptations behave as documented |
...
copies the eight bound Codex 0.144.1 presets unchanged, then appends configured
non-built-in LLM aliases in stable name order by copying `gpt-5.6-terra` and
replacing only `slug`, `display_name`, and `description`; Embedding groups are
not catalog entries. The gateway removes every active `model_catalog_json`
...
assignment. Gateway config, catalog, TOML, and hot activation use compensating
rollback. An upgrade must rebind the packaged asset and retest this ownership
...
rule. The Codex `0.144.1` source still carries an optional Namespace on
...
`src/codex_rosetta/gateway/admin/tool_catalog.json` is a read-only conceptual snapshot of fixed Codex tools. Its metadata is bound to Codex CLI `0.144.0` and source commit `2e8c3756f95789c215d9ea9a5ade6ec377934b3f`. It intentionally excludes `tool_search`, the obsolete hosted `image_generation` tool, and runtime-dynamic MCP, plugin, app, and connector tools. The one declarative exception is the GitHub Namespace Profile rule: it does not expose or create the namespace, and applies only when the installed GitHub connector supplies it. Current image generation is represented by the `image_gen.imagegen` namespace tool. The catalog does not describe the exact tools available to any individual request.
