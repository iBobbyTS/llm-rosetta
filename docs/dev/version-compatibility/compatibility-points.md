...
| Codex Search and Images endpoints | Expose JSON `POST /v1/alpha/search`, `/v1/images/generations`, and `/v1/images/edits`. `image_gen.imagegen` Passthrough retains direct Tool Mapping only routing; Modified resolves its Profile Base URL/Token and forwards unchanged OpenAI Images generation/edit JSON with the configured model alias across every LLM protocol, while Disabled rejects the endpoint. No vendor-private image API translation is attempted. The Responses web.run mapping Profile and converted routes locally implement atomic `search_query` through Tavily, direct-public-URL static HTML/plain-text `open` through a bounded Python fetcher, and fixed-offset `time`. Open revalidates public addressing on every redirect and returns normalized line-addressable text; stored references, browser rendering, compressed/non-text pages and the remaining unsupported commands/settings fail as structured HTTP 501 before partial execution. Gateway Logs record local request/result/error stages without image tokens | `gateway/codex_auxiliary.py`, `gateway/codex_images.py`, `gateway/codex_search.py`, `gateway/codex_page.py`, `gateway/tool_profiles.py`, `gateway/app.py`, `routing.py`, `test_codex_search.py`, `test_codex_page.py`, `test_codex_auxiliary.py`, `test_downstream_routes.py` | Codex changes endpoint paths, Images API body/response, SearchRequest commands/settings, required headers, SearchResponse shape, open reference/line semantics, or no longer includes routing model/session identity |
...
| Model-group tool profiles | Select immutable bundled `builtin`, `responses_pass_through`, or `responses_web_run_mapping` Profiles, or a complete user Profile, for every LLM model group. Tool Mapping only defaults to Responses pass through; the web.run preset changes only `namespace.web.run` and selects the local `/alpha/search` mapping. Supported paths apply per-tool disabled/passthrough/modified, namespace disabled/passthrough/expanded, and injection disabled/injected states. Function and Hosted items may define multiple localized text/password inputs or select inputs with distinct labels and persisted values; values remain inert unless a runtime feature explicitly consumes them. Modified `image_gen.imagegen` consumes its OpenAI Images Base URL/Token, while Modified `web_search` and `web.run` consume their independent search Provider/Token. Bundled Profiles omit hosted `image_generation` | `gateway/tool_profiles.py`, `gateway/config.py`, `gateway/codex_auxiliary.py`, Admin tools/model-group UI, `gateway/proxy.py` | Codex tool IDs, input-definition IDs/types/defaults/options, supported transformations, namespace expansion, injected localization dependencies, or Responses processing-mode semantics change |
| Static tool catalog | Package the fixed tool inventory, policy capabilities, optional Function/Hosted `profile_inputs`, and immutable Built-in Profile source, bound to Codex CLI `0.144.0` and source commit `2e8c3756f95789c215d9ea9a5ade6ec377934b3f`; exclude `tool_search`, obsolete hosted `image_generation`, and runtime-dynamic MCP/plugin/app/connector tools | `src/codex_rosetta/gateway/admin/tool_catalog.json` | Fixed tool sets, namespace members, profile input definitions, wire types, aliases, or model-controlled availability change |
...
| Codex Search and Images endpoints | Verify all three POST routes, model validation/aliasing, native pass-through path/body/status/header/error behavior, Profile-selected OpenAI Images Base URL/Bearer token forwarding for generation and edits on every LLM protocol, missing/invalid image configuration, Disabled handling, secret-free Gateway Logs, Profile-selected local Tavily search, bounded static direct-URL open and Python time behavior, public-address/redirect/content-type/size/line handling, domain/context/length mapping, atomic mixed-command rejection, and stable `not_implemented` errors | Invoke standalone `web.run` through Tool Mapping only with Responses pass through and confirm upstream forwarding; repeat with Responses web.run mapping and confirm successful local search, direct-URL open and time results, plus fatal 501 for stored-reference/browser navigation. Test a converted third-party model with Built-in separately. Invoke `image_gen.imagegen` generation and edit through a Modified Profile and confirm the selected OpenAI Images endpoint, model alias, response, and saved artifact |
...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, model-group resolution for every protocol, per-tool filtering, multi-value Function/Hosted text/password/select input persistence and select-option validation, namespace pass-through/expansion, injected-tool selection, absence of hosted `image_generation`, and `web.run`/`image_gen.imagegen` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Built-in, and a restrictive user Profile; verify direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, card inputs survive copy/save/reset, search cards use their own Tavily Tokens, select labels display while values persist, disabled tools disappear, Modified `image_gen.imagegen` selects its OpenAI Images endpoint without leaking its token, and supported adaptations behave as documented |
| Static tool catalog | Validate unique IDs, references, Function/Hosted input IDs/types/defaults/localization keys, Built-in Profile defaults, supported states, excluded dynamic tools and obsolete hosted `image_generation`, and exact CLI/source binding | Compare the catalog and Built-in Profile against tool definitions exposed by a real Codex session for the target version; record conditionally available tools separately |
