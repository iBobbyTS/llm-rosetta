...
| Deferred tool discovery | On Responses→Chat Code Mode routes, detect the live deferred-tool guidance in `exec.description` and expose one ordinary Chat `tool_search` Function with required `query`, optional `limit` (1–50, default 8), and optional `search_mode` (`natural_language` or `regex`). Translate the call deterministically into custom `exec` JavaScript that searches the request-local `ALL_TOOLS` array, ranks or regex-matches `name` plus full `description`, and returns bounded `{name, description}` matches. Keep raw `exec` available for the selected call. Do not hide source Namespaces, synthesize native `tool_search_call/output`, retain discovered definitions, or allocate window-scoped discovery state. Native Responses `tool_search_call/output` parsing remains a protocol compatibility path, not a local discovery cache | `gateway/code_mode_projection.py`, `gateway/tool_adaptation.py`, `test_code_mode_projection.py` | `ALL_TOOLS` stops being an Array of `{name, description}`, deferred guidance changes, the V8 runtime loses required Array/RegExp/String APIs or `text`/`exit`, Codex changes custom `exec` call/output semantics, or a direct tool conflicts with the synthetic name |
...
| Deferred tool and capability discovery | Synthetic Function exposure only with live deferred guidance; exact `query`/`limit`/`search_mode` schema; natural-language and regex script generation; invalid query/mode/limit/pattern bounds; direct-name conflict; custom `exec` round trip; raw `exec` retention; no Gateway window/cache state; ordered skill/plugin contextual metadata, explicit skill/plugin injection, implicit selected-skill read, and plugin provenance | Run `tests/live_agent/deferred_tool_search/01` through `07`: three explicit controls plus standalone-skill, standalone-MCP, plugin-skill, and plugin-MCP natural-language discovery. Require ordered skill/plugin context in rollout and converted requests; for MCP, require `tool_search` to translate to custom `exec`, the request-local ordered three-candidate `ALL_TOOLS` catalog, selected-call provenance, only the selected archive body/tool, a consumed result, and no implicit-prompt identifier leakage. Repeat at least one case with `search_mode=regex` and verify that a fresh request cannot reuse a prior search result through Gateway state |
...
deferred-tool guidance, Rosetta exposes an ordinary Chat `tool_search` Function.
