...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, model-group resolution for every protocol, per-tool filtering, namespace pass-through/expansion, injected-tool selection, and `web.run` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Built-in, and a restrictive user Profile; verify direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, disabled tools disappear, and supported adaptations behave as documented |
