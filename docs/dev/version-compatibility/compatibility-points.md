...
Codex `0.144.4` compatibility is **pending / not approved**. The controlled `deepseek-v4-flash` runs from earlier versions remain evidence only for the third-party Responses→Chat, Lite/code-mode, and successful `exec` paths they exercised. They are not native GPT or 0.144.4 auto-review evidence, and they do not close the required compact/resume/fork, plugin/MCP/deferred-tool, UI-phase, Desktop-tool, changed-error, or mapped/unmapped auto-review scenarios. Remote Compaction V2 now has a Rosetta coordinator: only configured OpenAI Responses passthrough plus `context_limit`/`user_requested` stays native; every other reason or route uses the bundled upstream compact prompt and principal-scoped seven-day mapping. Its source contract includes stable/default feature status, prompt hashes, string `encrypted_content`, trigger/item shape, metadata enums, and non-empty unequal `comp_hash` switching. The byte-identical coding-handoff quality scenario has now run through GPT on copied-config `Pixel (K12)` and `deepseek-v4-flash` on its sole route; both satisfied the protocol preconditions and preserved 8 of 11 fixed checks, so the test executor recorded both summaries as `ineffective`. This quality record is not an approval gate. The four protocol-only live cells remain required before approval. Responses WebSocket and incremental history remain unsupported. The current scoped review is recorded in [`reports/20260714-codex-v0.144.4.md`](reports/20260714-codex-v0.144.4.md); older reports remain historical evidence.
...
| Responses internal handling mode | Admin exposes one `responses` protocol. Official OpenAI selects the immutable all-Passthrough Profile and direct request/response bytes; OpenAI Custom and Custom + Custom select the direct `web.run`-injection Profile; listed third-party Responses providers select the mapping Profile and Responses→IR→Responses. Removed split protocol values are rejected during config loading. Direct mode changes only the selected Profile's tool behavior, preserves unknown non-tool fields and original response JSON/SSE bytes, and enforces bounded raw-stream limits | Confirm each Provider-selection branch chooses the documented Profile and processing path. Capture official OpenAI/relay direct requests with the Passthrough and `web.run`-injection Profiles, then treat third-party Responses normalization such as Qwen as unverified until real requests complete and continue |
...
| Context compaction resilience | orphan tool config, history trimming, compact fixture and window generation; protocol fixtures are separated from one byte-identical deterministic fact-retention scenario | Run the four protocol-only context/switch cells. Quality scenario completed separately: GPT native and DeepSeek Rosetta each installed exactly one compaction, resumed the same thread without another command/compaction, and preserved 8/11 fixed checks; both executor reviews are `ineffective` and non-gating |
...
Direct same-format Responses routing is an important forward compatibility strategy: after the selected Profile is applied, unknown non-tool fields are not compressed into the IR first, and the response is not reserialized. The Admin UI exposes one Responses protocol and derives the internal mode plus default Profile from the Provider selection: official OpenAI uses all-Passthrough, OpenAI/custom relays use the direct `web.run`-injection Profile, and listed third-party providers use the mapping Profile with Responses→IR→Responses. These branches must remain separately testable during upgrades even though they are no longer separate protocol options.
...
Direct Responses retains this body except for selected Profile tool changes. Listed-provider Responses normalization
...
`comp_hash` overlay uses only the configured upstream model name, falling back
to the exposed alias when no mapping exists. Provider identity is deliberately
excluded, aliases mapped to one upstream model share a hash, and changing the
upstream model changes the hash. Automated coverage must hold alias and Provider
inputs independently while varying the upstream name. A real Codex test must
