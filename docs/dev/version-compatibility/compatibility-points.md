...
| Responses transparent handling | Admin exposes one `responses` protocol. Official OpenAI selects the immutable all-Passthrough Profile, OpenAI Custom and Custom + Custom select `web.run` injection, and listed third-party providers select Tool Mapping; all of them use the same direct Responses transport. Removed split protocol values are rejected during config loading. Only Profile tool changes and model-switch plaintext compaction may alter an ordinary request; other non-tool fields, native compaction, response JSON, and SSE bytes are preserved, subject to bounded raw-stream limits and mandatory replacement of reflected configured Provider credentials. Exact, attested streaming requests preserve their original wire body; changed requests use rebuilt JSON without stale attestation. A validated native same-model compact tests raw-wire eligibility before Tool Profile and web-search adaptation, so those ordinary-request mutations cannot invalidate its signed body | Confirm each Provider-selection branch chooses the documented Profile while using the same direct transport. Capture OpenAI, relay, and listed-provider requests; verify byte-identical body plus attestation for unchanged native same-model compact even when its selected Profile would modify Responses Lite `additional_tools`, JSON fallback after Profile mutation on ordinary traffic, Profile-only differences for other requests, Rosetta plaintext rehydration across a Provider switch, and fake reflected credentials removed across every rotation position and raw-stream chunk split while all non-secret bytes remain identical |
...
rewritten except for mandatory configured-credential replacement across chunk
boundaries. A Codex upgrade that introduces larger required single events must
