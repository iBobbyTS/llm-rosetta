...
| Request and window identity | header/body metadata extraction, exact/+1 UTF-8 model and window identity budgets across source formats, rejection before routing/state allocation, correlation/state-key separation, sequential/concurrent no-window isolation, persistent-window continuity, principal-fair provider-metadata entry/byte quotas, and normal/error/cancel cleanup | Capture header/body/window changes and maximum observed identity lengths of real turn, compact, resume, fork, subagent |
