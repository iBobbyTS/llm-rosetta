...
| Model-group tool profiles | Select immutable bundled `builtin`, `responses_pass_through`, or `responses_web_run_mapping` Profiles, or a complete user Profile, for every LLM model group. Tool Mapping only defaults to Responses pass through; the web.run preset changes only `namespace.web.run` and selects the local `/alpha/search` mapping. Supported paths apply per-tool disabled/passthrough/modified, namespace disabled/passthrough/expanded, and injection disabled/injected states. Function items may define multiple localized text/password inputs whose values persist under the Profile and remain inert unless a runtime feature explicitly consumes them; Modified `image_gen.imagegen` consumes its OpenAI Images Base URL/Token. Bundled Profiles omit hosted `image_generation` | `gateway/tool_profiles.py`, `gateway/config.py`, `gateway/codex_auxiliary.py`, Admin tools/model-group UI, `gateway/proxy.py` | Codex tool IDs, input-definition IDs/types/defaults, supported transformations, namespace expansion, injected localization dependencies, or Responses processing-mode semantics change |
| Static tool catalog | Package the fixed tool inventory, policy capabilities, optional Function `profile_inputs`, and immutable Built-in Profile source, bound to Codex CLI `0.144.0` and source commit `2e8c3756f95789c215d9ea9a5ade6ec377934b3f`; exclude `tool_search`, obsolete hosted `image_generation`, and runtime-dynamic MCP/plugin/app/connector tools | `src/codex_rosetta/gateway/admin/tool_catalog.json` | Fixed tool sets, namespace members, profile input definitions, wire types, aliases, or model-controlled availability change |
...
| Model-group tool profiles | Validate all bundled/user Profile contracts, Admin CRUD and reference guards, model-group resolution for every protocol, per-tool filtering, multi-value Function text/password input persistence, namespace pass-through/expansion, injected-tool selection, absence of hosted `image_generation`, and `web.run`/`image_gen.imagegen` endpoint selection | Use real Codex sessions on Responses pass through, Responses web.run mapping, Built-in, and a restrictive user Profile; verify direct tools remain unchanged in the first preset, only `web.run` selects local Search in the second, Function inputs survive copy/save/reset, disabled tools disappear, Modified `image_gen.imagegen` selects its OpenAI Images endpoint without leaking its token, and supported adaptations behave as documented |
| Static tool catalog | Validate unique IDs, references, Function input IDs/types/defaults/localization keys, Built-in Profile defaults, supported states, excluded dynamic tools and obsolete hosted `image_generation`, and exact CLI/source binding | Compare the catalog and Built-in Profile against tool definitions exposed by a real Codex session for the target version; record conditionally available tools separately |
