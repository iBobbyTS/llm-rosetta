...
| Codex model catalog | Treat the bundled catalog and local catalog overrides as Codex client capability declarations, while Rosetta model groups remain the routing source of truth for alias, upstream model, protocol, and Tool Profile. The field reference covers every current catalog input, including the defaulted `effective_context_window_percent`, four bundled JSON keys ignored by Codex 0.144.1, prompt/template precedence, and safe third-party defaults. New third-party integrations prefer `web.run` over legacy hosted `web_search` and collaboration v2 over `multi_agent_v1`, after the newer lifecycle passes real tests | `../openai-codex-src/codex-rs/models-manager/models.json`, `../openai-codex-src/codex-rs/protocol/src/openai_models.rs`, `../openai-codex-src/codex-rs/models-manager/src/model_info.rs`, `docs/en/codex-model-catalog.md`, `docs/zh-cn/codex-model-catalog.md` | A Codex upgrade changes fields, nested types, enum values, serde defaults/skip behavior, fallback metadata, bundled model values, prompt precedence, or the requests/tools selected by those values; Rosetta aliases copied from built-in models may then advertise unsupported third-party behavior |
...
| Codex model catalog | Diff the complete bundled JSON key set and per-model values; extract `ModelInfo`, nested structs, enum wire values, serde rename/default/skip behavior, unknown-model fallback, instruction-template precedence, and each field's runtime consumers; verify the English/Chinese field inventories remain complete and synchronized | For every changed capability selector, run the real Codex client against the actual upstream behind the Rosetta alias and inspect Gateway Logs. Verify the declared command/tool mode, Responses Lite shape, reasoning, context/compact behavior, search, and multi-agent lifecycle. For a new integration, test `web.run` and collaboration v2 first; retain `web_search` or `multi_agent_v1` only when the newer surface is demonstrably unreliable |
