...
| Model-group tool profiles | Select the immutable bundled `builtin` profile or a complete user profile per LLM model group; apply per-tool disabled/passthrough/modified, namespace disabled/expanded, and Rosetta injection disabled/injected states before forwarding | `gateway/tool_profiles.py`, `gateway/config.py`, Admin tools/model-group UI, `gateway/proxy.py` | Codex tool IDs, supported transformations, namespace expansion, or injected localization dependencies change |
...
| Model-group tool profiles | Validate Built-in/user profile contracts, Admin CRUD and reference guards, model-group resolution, per-tool filtering, namespace expansion, injected-tool selection, and direct/Chat route behavior | Use real Codex sessions on Built-in and a restrictive user Profile; verify disabled tools disappear, expanded namespaces remain callable, and localized calls restore correctly |
