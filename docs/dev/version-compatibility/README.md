...
- Review Responses Pass through, Responses Rosetta, and Responses→Chat separately. The first naturally retains unknown fields; the latter two use IR and require explicit review for every new wire shape. Pass through and Rosetta are internal handling modes, not distinct wire protocols.
