...
- request-local `ALL_TOOLS` search exposure only with live deferred guidance; `query`/`limit`/`search_mode` schema and validation; natural-language/regex generated JavaScript; bounded version-marked complete match output; custom `exec` round trip; raw `exec` retention; direct-name conflict; and absence of Gateway discovery state. Verify paired request-history activation for the exact three Node REPL names, parseable same-name declarations, `js`-only helper exclusion, raw-`exec` activation without mapping-cache state, structured model arguments, JSON-safe outer-call generation, text/image/`isError` forwarding, streaming and non-streaming return, malformed/oversized result rejection, and same-named direct Function precedence. Keep native `tool_search_call/output` converter fixtures separately as protocol compatibility coverage;
