...
- request-local `ALL_TOOLS` search exposure only with live deferred guidance; `query`/`limit`/`search_mode` schema and validation; natural-language/regex generated JavaScript; version-marked output with a 24,000-character serialized budget, whole-match-only admission, oversized-match skip/backfill, and accurate `returned_matches`/`total_matches`/`truncated`; custom `exec` round trip; raw `exec` retention; direct-name conflict; and absence of Gateway discovery state. Verify paired request-history activation for the exact three Node REPL names, parseable same-name declarations, `js`-only helper exclusion, raw-`exec` activation without mapping-cache state, structured model arguments, JSON-safe outer-call generation, text/image/`isError` forwarding, streaming and non-streaming return, malformed/oversized result rejection, and same-named direct Function precedence. Verify model-facing history removes descriptions only for actually projected exact Node matches while leaving the source request, unknown matches, malformed declarations, unpaired results, and direct-name conflicts unchanged. Keep native `tool_search_call/output` converter fixtures separately as protocol compatibility coverage;
