...
- Verify model-group Tool Profiles on Tool Mapping only, Responses Rosetta, Chat, Anthropic, and Google routes, including Disabled filtering, Modified handling, multiple Function/Hosted text/password/select input persistence, bundled Profile input-only overrides with immutable delivery states, select label/value behavior, input `visible_when`, description default/custom state visibility, Modified-only descriptions for `request_user_input`/`create_goal`/`update_goal`, hidden-value retention, independent `web_search`/`web.run` Provider and Token execution, `image_gen.imagegen` OpenAI Images endpoint selection and token redaction, Namespace pass-through/expansion, Disabled Namespace child-state coercion and selector locking, Rosetta injection selection, and all bundled defaults including Chat Default disabling `multi_agent_v1`; verify Responses pass through preserves native tools and Responses web.run mapping changes only `web.run` endpoint handling;
...
- Chat Default Code Mode projection parses the live Codex declarations for `exec_command`, `write_stdin`, `update_plan`, `apply_patch`, `view_image`, `web.run`, Goal, and Clock tools; covers the Codex renderer's literal, union, intersection, array, tuple, object-index-signature, and normalized-heading forms; rejects unknown TypeScript tokens; emits ordinary Chat Functions only for valid declarations; rebuilds deterministic custom `exec` calls in streaming and non-streaming paths; routes localized `Edit`/`Write` through the extracted nested `apply_patch` projection when necessary; preserves same-named direct Functions; keeps `wait`, `request_user_input`, and `collaboration` direct; restores collaboration namespaces; and replays projected history from the encrypted mapping cache within TTL;
