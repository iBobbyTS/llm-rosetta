...
- Verify model-group Tool Profiles on Tool Mapping only, Responses Rosetta, Chat, Anthropic, and Google routes, including all four Admin categories, Disabled filtering, Modified handling, text/password/select persistence, bundled Profile input-only overrides with immutable delivery states, UI-hidden guidance, localized Modified summaries, independent `web_search`/`web.run` Provider and Token execution, `image_gen.imagegen` endpoint selection and token redaction, Namespace Expanded/Passthrough/Disabled behavior, Disabled child coercion and selector locking, and Rosetta injection selection. Verify Chat Default is the only bundled Profile, disables `multi_agent_v1` and upstream-visible `apply_patch`, marks `image_gen__imagegen` Modified with editable Base URL/Token fields, and injects exactly `Read`/`Glob`/`Grep` plus `Edit`/`Write`; use copied Profiles to test native pass-through and web.run mapping behavior;
...
- Chat Default Code Mode projection keeps the Disabled parent `exec` only through internal Responses-to-Chat conversion, parses the live Codex declarations for `exec_command`, `write_stdin`, `update_plan`, `apply_patch`, `view_image`, `web.run`, conditional `image_gen`, Goal, Clock, Memories, and Skills tools; covers the Codex renderer's literal, union, intersection, array, tuple, object-index-signature, and normalized-heading forms; rejects unknown TypeScript tokens; emits ordinary Chat Functions only for valid model-visible declarations; never invents `image_gen` when Codex's auth/feature/modality gates omit it; never re-exposes Disabled `exec` when parsing fails; uses the matching `text(...)`, `image(...)`, or `generatedImage(...)` result helper; keeps `apply_patch` model-hidden while retaining its internal projection for localized `Edit`/`Write`; rebuilds deterministic custom `exec` calls in streaming and non-streaming paths; preserves same-named direct Functions; keeps `wait`, `request_user_input`, and `collaboration` direct; restores collaboration namespaces; and replays projected history from the encrypted mapping cache within TTL;
...
- Static tool-catalog contract: unique IDs, valid placement/policy references, required fixed tools, direct Responses Namespace parents versus flat Code Mode `namespace__function` entries, Function/Hosted input IDs/types/defaults/localization keys, excluded runtime-dynamic plugin/MCP/app/connector tools and obsolete hosted `image_generation`, current `image_gen__imagegen` coverage, Chat Default Profile defaults, supported states, and exact CLI/source metadata binding.
...
- Confirm that Responses Tool Mapping only with a copied all-Passthrough Profile does not lose fields, and that a copied Profile with only `web.run` Modified changes only Search endpoint handling; record Responses Rosetta with the intended third-party provider as unverified unless it completes through IR and can be continued; confirm the Chat bridge does not leak Rosetta's internal metadata to the upstream;
