...
- The single Admin Responses protocol selects official OpenAI all-Passthrough, OpenAI/custom-relay direct `web.run` injection, or listed-provider Responses→IR→Responses mapping as documented; direct routes retain unknown non-tool fields and preserve original response JSON/SSE bytes below the transport safety envelope;
...
- Confirm that the bundled 透传 Profile does not lose fields or alter any native tool shape, and that the bundled web.run 注入 Profile changes only Search endpoint handling; record listed-provider Responses normalization with the intended third-party provider as unverified unless it completes through IR and can be continued; confirm the Chat bridge does not leak Rosetta's internal metadata to the upstream;
