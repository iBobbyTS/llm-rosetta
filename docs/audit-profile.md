...
- Privacy/data impact: High. Prompts, source code, tool calls, compaction summaries, and request diagnostics may be retained. Diagnostics redact all configured token values and token-shaped fields; untrusted return gates redact the current provider/client inventory defined above. Non-token diagnostic content may remain.
...
- Risk tolerance and explicit non-goals: tolerate pre-release gaps in live-provider evidence, production operations, HA, and recovery. Do not tolerate leakage of credentials configured for the active outbound client, credentials into diagnostics, cross-API-key state, silent Codex protocol corruption, unsafe Admin/API-key bypass, or unbounded state growth on the supported path. Cross-provider/client credential reflection is accepted outside the active return-gate domain for the local/LAN-only deployment boundary.
...
- Required security controls: mandatory Admin password and at least one Gateway API key; no unauthenticated Rosetta mode; fail-closed `/v1` auth; strict Admin auth/CORS; stable principal scoping; token-only redaction, including the active provider/client credentials reflected in success, error, or transport-exception content; return gates use only the active outbound provider/client credential inventory, while diagnostics use the complete configured-token inventory; credentials have no minimum-length restriction, so ambiguous in-domain collisions must fail closed rather than leak the credential or silently emit corrupted SSE/JSON; owner-only secret/database paths; bounded request/tool state; URL-authoritative preset/custom provider routing; runtime-only missing-or-unrecognized-`api_type` inference with a terminal warning; arbitrary HTTP(S) custom egress accepted within the local/LAN boundary; provider redirects denied by default and isolated by redirect policy, with explicit per-provider opt-in only; auxiliary HTTP redirects denied; no live API calls in audit; live runners fail closed without explicit developer approval.
