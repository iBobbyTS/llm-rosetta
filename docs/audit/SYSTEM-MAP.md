...
| Provider transport | URL/auth header construction, canonical key rotation, client pool/SSE, credential return boundary | Project | Critical | `ProviderInfo`, `HttpTransport`, `CredentialRedactingTransport`, proxy passthrough/converted handlers | connection pools, key ring, actual wire credentials; JSON/SSE semantic escape handling and active-provider/client credential-domain ownership are explicit | vendored HTTP client | local process/container | `gateway/transport/`, `providers.py`, `gateway/proxy.py`, credential-return tests, AUD-019/AUD-020 |
...
| IF-03 | Gateway | upstream provider | OpenAI Chat/Responses, Anthropic Messages, Google GenAI HTTP | exact backend-supported or runtime-inferred `api_type`; URL-authoritative preset or custom endpoint | provider credential egress; arbitrary custom HTTP(S) egress accepted only within local/LAN scope; redirects denied by default and allowed only by explicit provider opt-in; return collisions cover semantic JSON/SSE representations of the active provider credentials only | Gateway/provider owner + project owner | `config.py`, `providers.py`, `transport/`, `proxy.py`, converters, AUD-019/AUD-020 |
...
| Provider egress | Gateway process | converted request/model/config | provider API key in configured provider | prompt/tool payload and provider credential | request to configured preset or arbitrary custom HTTP(S) base URL/proxy; redirects fail closed by default, but explicit `allow_redirects` permits provider and model-discovery follow-ups; semantic return collisions are checked only against the active provider/client inventory | operator configuration; custom egress, opt-in redirect expansion, and cross-provider reflection accepted for local/LAN only; arbitrary credential lengths remain supported | `GatewayConfig`, `ProviderInfo`, proxy, Admin discovery, redirect/redaction tests, profile, AUD-019/AUD-020 |
...
| SCN-03 | Correctness/reliability/security | Codex Responses request routed to preset upstream | app → proxy → route/shim → converter → transport | target body and returned response preserve required fields without returning credentials under an owner-approved credential contract | deterministic converter/fake-upstream, reflected-credential, and colliding-token tests | Critical | Compatibility/core/transport/project owner |
| SCN-04 | Correctness/reliability | Provider SSE stream has deltas, tool calls, usage, EOF/cancel | stream processor, phase buffer, SSE formatter, raw credential redactor | ordered downstream fields/events, valid framing, terminal state, resource cleanup | stream/lifecycle and colliding-token framing tests | Critical | Gateway/core |
...
| `GatewayConfig`/provider resolution/transport | SCN-03/04/09/10 | route, protocol inference, credential egress/rotation, and return redaction depend on parsed config, accepted credential domain, wire framing, and HTTP policy | config schema, provider/shim registry, credential parsing/validation, redactor propagation, admin writes, raw/parsed stream behavior | stop only after actual wire credentials, colliding legitimate content, and all untrusted return channels are reconciled |
...
| Semantically equivalent JSON encodings bypass raw collision matching | Unicode and solidus escapes could decode downstream to an exact configured credential after passing the byte gate | shared exact-wire plus parsed JSON/SSE semantic validation and adversarial encoding tests | Closed / AUD-019; deterministic only |
