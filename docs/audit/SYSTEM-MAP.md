...
| Optional web/search/image bridges | external search, web-run sidecar, image fetch/generation | Project | High | search/image routes and sidecar supervisor | health state, fetched content, tool references; configured Tavily, sidecar, and auxiliary provider credentials are removed from untrusted returns | Tavily/sidecar/provider endpoints | optional local/LAN | `gateway/web_run*`, `web_search.py`, `codex_images.py`, sidecar/auxiliary tests |
...
| IF-03 | Gateway | upstream provider | OpenAI Chat/Responses, Anthropic Messages, Google GenAI HTTP | exact backend-supported or runtime-inferred `api_type`; URL-authoritative preset or custom endpoint | provider credential egress; arbitrary custom HTTP(S) egress accepted only within local/LAN scope; redirects denied by default and allowed only by explicit provider opt-in; untrusted provider returns cross an exact configured-credential redaction boundary | Gateway/provider owner | `config.py`, `providers.py`, `transport/`, `proxy.py`, converters |
...
| Provider egress | Gateway process | converted request/model/config | provider API key in configured provider | prompt/tool payload and provider credential | request to configured preset or arbitrary custom HTTP(S) base URL/proxy; redirects fail closed by default, but explicit `allow_redirects` permits provider and model-discovery follow-ups; all provider/Admin discovery returns remove configured credentials before client/model/diagnostic use | operator configuration; custom egress and opt-in redirect expansion accepted for local/LAN only; reflected-return control verified deterministically | `GatewayConfig`, `ProviderInfo`, proxy, Admin discovery, redirect/redaction tests, profile |
...
| `GatewayConfig`/provider resolution/transport | SCN-03/09/10 | route, protocol inference, credential egress/rotation, and return redaction depend on parsed config and HTTP policy | config schema, provider/shim registry, credential parsing, redactor propagation, admin writes, HTTP client behavior | stop only after actual wire credentials and all untrusted return channels are reconciled |
