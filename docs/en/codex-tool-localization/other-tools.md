...
Calls to projected Functions are rebuilt as deterministic JavaScript calls on the nested `tools` object and returned to Codex as `custom_tool_call` calls to `exec`. The exact Chat-to-Codex call mapping is stored in the existing encrypted tool-history cache. A successful history read restores the original Chat Function and arguments and renews its 24-hour TTL from that read. `view_image` forwards its result through `image(...)`, `image_gen.imagegen` uses `generatedImage(...)`, and text-bearing projected tools use `text(...)`.
