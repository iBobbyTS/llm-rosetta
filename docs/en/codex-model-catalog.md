...
| `supports_search_tool` | boolean, `false` | Controls Codex namespace/tool discovery such as native `tool_search`; it is distinct from hosted `web_search` and standalone `web.run`. | Enable only after the client-visible discovery flow is tested. On Responses→Chat Code Mode routes, Rosetta separately projects request-local `ALL_TOOLS` search when the live `exec` description advertises deferred tools. Exact Node REPL matches are restored from paired request history as structured functions and converted back to `exec`; no Gateway namespace/discovery cache is retained. Configure hosted search in Tool Profiles and Modified `web.run` in the global Web Search settings. |
