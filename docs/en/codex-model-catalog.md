...
| `supports_search_tool` | boolean, `false` | Controls Codex namespace/tool discovery such as `tool_search`; it is distinct from hosted `web_search` and standalone `web.run`. | Enable only if the client-visible discovery flow and Rosetta namespace restoration are tested. Configure hosted search in Tool Profiles and Modified `web.run` in the global Web Search settings. |
