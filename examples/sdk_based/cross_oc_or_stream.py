#!/usr/bin/env python3
"""Cross-provider multi-turn conversation: OpenAI Chat <-> OpenAI Responses (SDK, Stream).
...
Odd turns use OpenAI Chat Completions API, even turns use OpenAI
Responses API.
...
Usage:
    ...
    proxychains -q python examples/sdk_based/cross_oc_or_stream.py

    # Or if OpenAI is directly accessible
    python examples/sdk_based/cross_oc_or_stream.py
...
def init_openai_chat_client():
    """Initialize OpenAI Chat SDK client and return (client, model).
...
def main():
    """Run cross-provider multi-turn streaming conversation between OpenAI Chat and OpenAI Responses."""
    print("=" * 60)
    print("Cross-Provider Multi-Turn Conversation (Stream)")
    print("OpenAI Chat <-> OpenAI Responses (SDK)")
    print("=" * 60)

    # Initialize clients
    oc_client, oc_model = init_openai_chat_client()
