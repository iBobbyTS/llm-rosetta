#!/usr/bin/env python3
"""Cross-provider multi-turn conversation: OpenAI Chat <-> OpenAI Responses (SDK).
...
Completions API, even turns use OpenAI Responses API.

Conversation covers: text, images, and tool calls.

Usage:
    # OpenAI requires proxy in restricted networks
    proxychains -q python examples/sdk_based/cross_oc_or.py

    # Or if OpenAI is directly accessible
    python examples/sdk_based/cross_oc_or.py
...
def main():
    """Run cross-provider multi-turn conversation between OpenAI Chat and OpenAI Responses."""
    print("=" * 60)
